...
void display() {
    ...
    vec3 cam_pos(2*cos(time), 2*sin(time)+.4, -2.0f);
    vec3 cam_look(0.0f, 0.0f, 0.0f);
    vec3 cam_up(0.0f, 0.0f, 1.0f);
    mat4 view = Eigen::lookAt(cam_pos, vec3(1.0, 0.5, 0.0), cam_up);
    ...
    mat4 view_mirror = Eigen::lookAt(cam_pos_mirror, vec3(1.0, 0.5, 0.0), cam_up);
    fb.bind();
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        mesh.draw(model, view_mirror, projection);
    fb.unbind();
